...
def test_subject(data_dict, surf_dict, subid, exp, surf_paths):
    ...
    for key in SIGNALS: 
        ...
        if not op.exists(path) or REFRESH:
            ...
            REF_SPC.save_image(v2n2v_tob_sp, pathroot + '_v2n2v_tob_sp.nii.gz')
        ...
        path = pathroot + '_s2v2n_tob.func.gii'
        if not op.exists(path) or REFRESH:
            s2v2n_tob = v2n_tob_mat.dot(n2v_tob)
            savefunc(s2v2n_tob, path)

        path = pathroot + '_s2v2n_tob_sp.func.gii'
        if not op.exists(path) or REFRESH:
        ...
        path = pathroot + '_s2v2n_wb.func.gii'
        if not op.exists(path) or REFRESH:
            s2v2n_wb = v2n_wb_mat.dot(n2v_wb)
            savefunc(s2v2n_wb, path)

        path = pathroot + '_s2v2n_wb_sp.func.gii'
        if not op.exists(path) or REFRESH:
    ...
    for exp in EXPERIMENTS:
        for idx, subid in enumerate(SUBIDS()[:N_SUBS]):
            ...
            if not op.exists(p_name):
                ...
                proj = projection.Projector(L_hemi, REF_SPC, FACTOR)
            ...
            # Saving the ground truths (if needed)
            for key in SIGNALS:
                vol = vol_truths[key]
